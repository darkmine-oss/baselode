...
def load_unified_dataset(assays_source, structures_source, source_column_map=None, **kwargs):
    ...
    - **Structural rows** (point schema): ``depth`` is the measured depth.
      ``from``, ``to`` and ``mid`` are all set to ``depth`` so the interval-based
      strip-log renderer can consume them.  Rows are tagged ``_source = 'structural'``.
    ...
    **kwargs:
    ...
    # `to = depth + 0.001` gives a non-zero interval width satisfying the
    # view layer requirement that to > from.
    if not struct_df.empty and DEPTH in struct_df.columns:
        struct_df[FROM] = struct_df[DEPTH]
        struct_df[TO] = struct_df[DEPTH] + 0.001
