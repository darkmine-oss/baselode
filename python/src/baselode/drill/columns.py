...
HIDDEN_COLUMNS = {
    ...
    "datasource_hole_id",
    "anumber", "collarid", "companyholeid", "company_hole_id", "company_id", "companyholeid_x", "companyholeid_y",
    # Project codes
...
# natural representation (dip head + azimuth tail).
TADPOLE_COLUMNS = {"dip", "alpha"}
...
def classify_columns(df):
    ...
    for col in df.columns:
        ...
        # Tadpole-type: structural orientation columns (dip, alpha)
        if normalized in TADPOLE_COLUMNS:
            series = df[col]
            if not series.isna().all():
                numeric = pd.to_numeric(series, errors="coerce")
                if numeric.notna().any():
                    by_type[col] = DISPLAY_TADPOLE
                    continue
    ...
    tadpole_cols = [c for c, t in by_type.items() if t == DISPLAY_TADPOLE]
