...
export function buildCollarMarkers(holesMeta, singlePointHoles = [], opts = {}) {
  ...
  const entries = [
    ...holesMeta.map((h) => ({ point: h.collar, index: h.index, id: h.id })),
    ...singlePointHoles.map((h, i) => ({ point: h.point, index: holesMeta.length + i, id: h.id })),
  ];
  ...
  entries.forEach((e, i) => {
    const p = e.point.clone().add(lift);
    m.compose(p, q, s);
    disc.setMatrixAt(i, m);
    disc.setColorAt(i, holeBaseColor(e.index));
...
export function buildHoleLabels(holesMeta, singlePointHoles = [], opts = {}) {
  ...
  entries.forEach((e) => {
    ...
    if (!sprite) return;
    sprite.position.copy(e.point).add(new THREE.Vector3(0, 0, lift));
...
export function buildCollarDropLines(holesMeta, singlePointHoles, datumZ, opts = {}) {
  ...
  const push = (p) => {
    ...
    pts.push(p.x, p.y, p.z, p.x, p.y, datumZ);
  };
