...
def test_plot_numeric_and_categorical_traces():
    ...
    assert len(cat_fig.data) == 2  # one bar trace per unique label ("a", "b")
...
def test_plot_drillhole_trace_variants():
    ...
    assert len(fig_cat.data) == 2  # one bar trace per unique label ("a", "b")
...
def test_load_geology_standardizes_common_fields():
    ...
    assert "comments" in loaded.columns
    assert loaded["mid"].tolist() == [5.0, 15.0]


def test_plot_geology_strip_log_bar_traces():
    geology = pd.DataFrame({
        "from": [0, 10],
        "to": [10, 20],
    ...
    # One bar trace per unique geology code
    assert len(fig.data) == 2
    assert all(trace.type == "bar" for trace in fig.data)
    assert {trace.name for trace in fig.data} == {"FG", "SBIF"}
...
def test_plot_core_photo_striplog_places_images():
    images = [
    ...
    fig = view.plot_core_photo_striplog(images)
...
def test_plot_core_photo_striplog_empty_returns_figure():
    import plotly.graph_objects as go
    fig = view.plot_core_photo_striplog([])
    assert isinstance(fig, go.Figure)
    assert len(fig.layout.images) == 0


def test_plot_core_photo_striplog_dataframe():
    images = pd.DataFrame({
        "from": [0, 10],
        "to": [10, 20],
...
def test_plot_core_photo_striplog_skips_invalid_intervals():
    images = [
...
def test_plot_core_photo_striplog_skips_missing_url():
    images = [
...
def test_plot_core_photo_striplog_depth_range():
    images = [{"from": 5, "to": 15, "image_url": "https://example.com/core.jpg"}]
    fig = view.plot_core_photo_striplog(images, depth_range=(0, 50))
    anchor = fig.data[0]
