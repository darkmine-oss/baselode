...
def test_load_structures_raises_without_depth():
...
def test_validate_structural_intervals():
    df = pd.DataFrame({
        ...
        "from": [20.0, 45.0, 10.0, 60.0],
        "to": [25.0, 55.0, 18.0, 75.0],
        "dip": [60.0, 75.0, 30.0, 85.0],
        "azimuth": [90.0, 180.0, 270.0, 45.0],
    })
...
def test_plot_structural_strip():
    df = pd.DataFrame({
        "from": [20.0, 45.0, 10.0, 60.0],
        "to": [25.0, 55.0, 18.0, 75.0],
        "structure_type": ["shear zone", "fault zone", "shear zone", "fault zone"],
    })
...
def test_plot_comments_log():
    df = pd.DataFrame({
        "from": [20.0, 45.0, 10.0, 60.0],
        "to": [25.0, 55.0, 18.0, 75.0],
        "comments": ["broken core", "clay gouge present", "minor fracturing", "brecciated"],
    })
    fig = plot_comments_log(df, from_col="from", to_col="to", comment_col="comments")
    assert fig is not None
    # Shapes = one rectangle per interval
    assert len(fig.layout.shapes) == len(df)


def test_plot_comments_log_text_wrapping():
    df = pd.DataFrame({
        "from": [0.0, 10.0],
        "to": [10.0, 20.0],
        "comments": ["This is a long comment that should wrap across multiple lines", ""],
    })
    fig = plot_comments_log(df, chars_per_line=20)
    assert fig is not None
    # Uncommented intervals do not render at all; data[0] is the full-width
    # invisible hover bar over the commented interval, data[1] its inline text.
    hover_bar, text_trace = fig.data
    assert hover_bar.type == "bar"
    assert len(hover_bar.hovertext) == 1
...
def test_plot_comments_log_truncates_text_to_the_interval_line_budget():
    ...
    df = pd.DataFrame({
        ...
        "to": [2.0, 60.0],
        "comments": [
            "A very long comment in a two metre sliver of a sixty metre track "
...
def test_column_normalization_computed_plane_dip_to_dip():
    """'Computed_Plane_Dip' column → 'dip' in standardized output."""
    df_raw = pd.DataFrame({
        "HoleId": ["A"],
        "Depth": [10.0],
        "Computed_Plane_Dip": [45.0],
        "DipDir": [270.0],
    })
