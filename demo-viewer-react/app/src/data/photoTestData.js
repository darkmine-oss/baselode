...
import { buildTrayPhotos } from 'baselode';
...
export const NVCL_09RTD001_TRAYS = [
...
//   DS1  93 trays  0.004 – 324.188 m
//   DS2  58 trays  324.204 – 537.1 m
// Symlinked into public/data/nvcl/12CADD001/{ds1,ds2}/{thumb,full}

export const NVCL_12CADD001_DS1_TRAYS = [
...
export const NVCL_12CADD001_DS2_TRAYS = [
...
export const nvclFilename = (i) => `mosaic_${String(i).padStart(3, '0')}.jpg`;

// ── Synthetic demo data ───────────────────────────────────────────────────
//
// SVG placeholders with depth-varying colours, demonstrating multi-set
// (Wet / Dry) side-by-side layout.

const BOX_INTERVAL_M = 1;
export const DEMO_HOLES = ['GSWA-001', 'GSWA-002', 'GSWA-003'];
export const DEMO_HOLE_DEPTHS = { 'GSWA-001': 60, 'GSWA-002': 40, 'GSWA-003': 80 };
...
export function generateDemoPhotos(holeId, maxDepth) {
  const photos = [];
...
export const DEMO_DATA = Object.fromEntries(
  DEMO_HOLES.map((id) => [id, generateDemoPhotos(id, DEMO_HOLE_DEPTHS[id])]),
);

// Combine both NVCL 12CADD001 sections into one continuous photos-table array
export const NVCL_12CADD001_PHOTOS = [
...
// ── CorePhotoTable photo-array builder ────────────────────────────────────

export function getHolePhotos(holeId) {
  if (!holeId) return [];
  if (holeId === '09RTD001') {
...
// ── Strip-log trace-grid photo-hole builder ───────────────────────────────
//
// Converts the photo-table records (from_depth/to_depth/image_url) into the
// trace-grid hole shape: { id, points: [{ from, to, image_url, image_mode }] }.
// Each photo set on the same hole becomes its own hole so each renders as a
// separate, independently-selectable strip-log track.

function photosToTracePoints(photos) {
  return photos.map((p) => ({
    from: p.from_depth,
    to: p.to_depth,
...
/**
 * Build photo "holes" suitable for the trace-grid `extraHoles` input.
 *
 * Returns: [
 *   { id: '09RTD001 (photos)',    points: [...] },
 *   { id: '12CADD001 (photos)',   points: [...] },
 *   { id: 'GSWA-001 (Wet photos)', points: [...] },
 *   { id: 'GSWA-001 (Dry photos)', points: [...] },
 *   ...
 * ]
 *
 * Each entry is a separate hole so the strip-log dropdown lists each photo
 * set independently. Picking one shows an `image_url` property; the strip
 * log dispatches it to buildCorePhotoConfig automatically.
 */
export function buildPhotoTraceHoles() {
  const holes = [];

  const nvcl09 = getHolePhotos('09RTD001');
  if (nvcl09.length) {
    holes.push({ id: '09RTD001 (photos)', points: photosToTracePoints(nvcl09) });
  }

  const nvcl12 = getHolePhotos('12CADD001');
  if (nvcl12.length) {
    holes.push({ id: '12CADD001 (photos)', points: photosToTracePoints(nvcl12) });
  }

  for (const demoHoleId of DEMO_HOLES) {
    const photos = DEMO_DATA[demoHoleId] || [];
    const bySet = new Map();
    for (const p of photos) {
      const setName = p.photo_set || 'photos';
      if (!bySet.has(setName)) bySet.set(setName, []);
      bySet.get(setName).push(p);
    }
    for (const [setName, setPhotos] of bySet) {
      holes.push({
        id: `${demoHoleId} (${setName} photos)`,
        points: photosToTracePoints(setPhotos),
      });
    }
  }
